...
const Form = params => {
  ...
  const fieldHTML = fields.map((group, key) => {
    ...
    if( group[0].field === 'submit' ) return <button 
      className="govuk-button lbh-button" key={key} onClick={() => setValidate(true) }>{group[0].text}</button>

    const grouping = group.map((field, key) => {
      
      const id = `${field.name}_${key}`

      const fieldChange = (name, val, type) => {
        const newData = { ...data }
        newData[name] = type === 'number' ? Number(val) !== 0 ? Number(val) : null : val
        setData(newData)
      }
      ...
      return <Fragment key={id}>
        ...
        { field.label && <label className="govuk-label lbh-label" htmlFor={id}>{field.label}</label> }

        { field.field === 'input' && <input 
          className={`govuk-input lbh-input w-100 govuk-input--${fieldStatus}`}
          value={ data[field.name] ?? '' } 
          onChange={e => fieldChange(field.name, e.target.value, field.type)}
          id={id} 
        ...
        { field.field === 'datepicker' && <DatePicker
        ...
        {field.field  === 'select' && <select
          className={`govuk-input lbh-input w-100 govuk-input--${fieldStatus}`}
          value={data[field.name]} 
          onChange={e => fieldChange(field.name, e.target.value, 'select')}
          id={id} 
          name={field.name} 
          required={field.required}
          aria-describedby="input-with-error-message-hint input-with-error-message-error"
        >
          ...
          {field.options.map(option => { return <option key={option[0]} value={option[0]}>{option[1]}</option> })}
        </select>}

        { fieldStatus === 'error' && <span id="input-with-error-message-error" className='govuk-error-message lbh-error-message'>
          <span className='govuk-visually-hidden'>Error:</span> { field.label ? `"${field.label}"` : 'This field' } is required
        </span> }
    ...
    return <div className={`govuk-form-group lbh-form-group govuk-form-group--${groupStatus}`} key={key}>
  ...
  return <form style={{maxWidth: '480px'}} onSubmit={e => onSubmit(e)}>
    {fieldHTML}
  </form>
