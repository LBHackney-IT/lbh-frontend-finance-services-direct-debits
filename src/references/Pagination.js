...
const Pagination = params => {
  ...
  const range = (start, end, step) => Array.from(Array.from(Array(Math.ceil((end-start) / step)).keys()), x => start+ x * step)
  
  return <>
    <div className="pagination_wrap">
      <ul className="pagination">
        {range(beginning, finish, 1).map(num => {
          return <li key={`pagination_${num}`} className={`page-item${num === page ? ' active' : ''}`}>
            <Link 
              className="page-link"
              to={`${prefix}/${num}`}
            >{num}</Link>
