...
const Home = () => {
  ...
  const runSearch = () => {
    ...
    history.push(`${RouteConstants.SEARCH}/${searchType}/${encodeURIComponent(searchTerm)}/1`)
  }
  ...
  return <>
    ...
    <div className="govuk-grid-row">
      ...
      <div className="govuk-grid-column-one-third" style={{ textAlign: 'right' }}>
        <Link to={RouteConstants.DIRECTDEBIT} className="mt-0 govuk-button lbh-button">{TextReferences.Titles.DirectDebits}</Link>
      </div>
    </div>
  </>
}
