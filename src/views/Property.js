...
import { getProperty, getPerson, getDirectDebits } from '../routes/Api'
...
const Property = () => {
      ...
      <table className='govuk-table lbh-table'>
        ...
        <tbody className='govuk-table__body'>{
          tenants.map(tenant => {
            ...
            const address = tenant.tenures.filter(tenure => tenure.id === PropertyId )
            const directDebitLink = () => history.push(`${RouteConstants.DIRECTDEBIT}/${tenant.id}/create`)
            
            return(<tr className='govuk-table__row' key={tenant.id}>
              <td className='govuk-table__cell' data-address={JSON.stringify(address)}>
                <Link 
                  className='lbh-link' 
                  to={`${RouteConstants.TENANT}/${tenant.id}`}
                >
                  {tenant.preferredTitle ? tenant.preferredTitle : tenant.title} {tenant.preferredFirstName ? tenant.preferredFirstName : tenant.firstName} {tenant.preferredSurname ? tenant.preferredSurname : tenant.surname}
                </Link>
              </td>
              <td className='govuk-table__cell'>{address[0].uprn}</td>
              <td className='govuk-table__cell'>{address[0].type}</td>
              <td className='govuk-table__cell'>{DateFormat(address[0].startDate)} - {address[0].endDate ? `${DateFormat(address[0].endDate)}` : 'Current'}</td>
              <td className='govuk-table__cell'>{DateFormat(address[0].startDate)}</td>
  ...
  const directDebitView = () => {
    ...
    if( directDebits !== undefined && directDebits.totalCount === 0 ) results = <>
      <p>No direct debits setup yet.</p>
    </>
    if( directDebits !== undefined && directDebits.totalCount !== 0 ) results = <p>Results.</p>

    return <>{intro}{results}<hr /></>
  ...
  const PropertyView = () => {
    ...
    if( property === undefined ) return
    if( property === null ) return <h4>{TextReferences.TextRef.NoTenantRecords}</h4>
    
    const TotalCharge = () => {
      if( !property.charges ) return 0
      return property.charges.combinedRentCharges + 
    ...
    return <>
      <h1>{TextReferences.Titles.Property}: {property.tenuredAsset.fullAddress}</h1>
      <p>ID: {property.id}</p>
      { property.charges && <>
        <h2>{TextReferences.TextRef.Financial}</h2>
        {descriptionList([
          { key: 'Current Balance', val: CurrencyFormat(property.charges.currentBalance) },
          { key: 'Rent', val: CurrencyFormat(property.charges.rent) },
          { key: 'Year to Date', val: '' },
          { key: `${property.charges.billingFrequency} ${TextReferences.TextRef.TotalCharges}`, val: CurrencyFormat(TotalCharge()) },
          { key: 'Service Charge', val: CurrencyFormat(property.charges.serviceCharge) },
      ...
      {descriptionList([
        ...
        { key: 'Tenancy Start Date', val: DateFormat(property.startOfTenureDate) },
        { key: 'Property Reference #', val: property.tenuredAsset.propertyReference },
      ])}    
      {directDebitView()}
    </>
