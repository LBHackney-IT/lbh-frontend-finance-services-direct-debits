...
resource "aws_s3_bucket" "frontend-bucket-production" {
  bucket = "lbh-housing-finance-details-frontend-production.hackney.gov.uk"
  acl    = "private"
  versioning {
    enabled = true
  }
  ...
  cors_rule {
    allowed_headers = ["*"]
    allowed_methods = ["GET"]
    allowed_origins = ["https://hfs.hackney.gov.uk"]
...
resource "aws_ssm_parameter" "cdn" {
  name  = "/housing-finance-details/production/app-url"
  type  = "String"
