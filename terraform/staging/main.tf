...
resource "aws_s3_bucket" "frontend-bucket-staging" {
  bucket = "lbh-housing-finance-details-frontend-staging.hackney.gov.uk"
  acl    = "private"
  versioning {
    enabled = true
  }
  ...
  cors_rule {
    allowed_headers = ["*"]
    allowed_methods = ["GET"]
    allowed_origins = ["https://hfs-staging.hackney.gov.uk"]
...
resource "aws_ssm_parameter" "cdn" {
  name  = "/housing-finance-details/staging/app-url"
  type  = "String"
